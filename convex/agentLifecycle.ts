...
export const getSystemHealth = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Step 2: Get all agent statuses at once (N+1 fix: bulk query instead of per-agent loop)
    const allStatuses = await ctx.db.query("agent_status").collect();
    const agentStatuses: any = {};
    for (const status of allStatuses) {
      agentStatuses[status.agentId] = status;
    }
...
export const getAgentMetrics = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Get recent executions (bounded to 200 to prevent unbounded collection)
    const executions = await ctx.db
      .query("executions")
      .withIndex("by_agent", (q) => q.eq("agentId", args.agentId) as any)
      .take(200);
...
/**
 * PHASE 5: getSystemHealthFixed() - Query (N+1 Bug Fix)
 *
 * Get system health with fixed N+1 query pattern.
 * Collects all agent_status rows at once and joins in memory,
...
export const getSystemHealthFixed = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Get all agent statuses at once (fixed: no N+1)
    const allStatuses = await ctx.db.query("agent_status").collect();
    const statusByAgentId: Record<string, any> = {};
    for (const status of allStatuses) {
      statusByAgentId[status.agentId] = status;
    }

    // Delegate to existing calculateSystemHealth pure function
    // Note: calculateSystemHealth is imported from execution-logic.ts
    // For now, we replicate the logic here to maintain independence
    ...
    for (const agent of agents) {
      const status = statusByAgentId[agent._id];
