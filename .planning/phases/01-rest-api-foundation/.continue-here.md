...
1. **Convex Backend Integration** (blocking full build)
   - Ensure all referenced Convex mutations exist in schema
   - Verify mutations: recordHeartbeat, pollWork, rotateKey, etc.
   - Test with running Convex backend (`npm run convex:dev`)

2. **Final Validation**
   - Build passes completely: `npm run build`
   - All tests passing: `npm test`
   - E2E smoke tests work: `npm run e2e`
   - Both `/api/` and `/api/v1/` paths return identical responses
