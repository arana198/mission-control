...
- `workspaces.ts` — Workspace CRUD itself can't use workspace-scoped builders (chicken-egg). Keep raw query/mutation except for workspace-internal operations.
...
NOTE: Some files may still need raw `query` or `mutation` for non-workspace-scoped functions alongside the workspace-scoped ones. That's acceptable -- the key is that workspace-scoped functions use the builders.
  ...
  <verify>
    <automated>cd /Users/arana/dev/ankit/mission-control && npm test -- --testPathPattern="workspace-isolation|documents|decisions|notifications|wiki|taskComments|executionLog"</automated>
  </verify>
  <done>
    - All 6 remaining files migrated to workspace-scoped builders
    - workspace-isolation.test.ts passes with 6+ tests covering isolation boundaries
