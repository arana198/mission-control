...
**IMPORTANT:** Use `ApiError` from `../lib/errors` for all error throwing. Follow the existing pattern in `workspaces.ts` with `wrapConvexHandler`.
  ...
  <verify>
    <automated>cd /Users/arana/dev/ankit/mission-control && npm test -- --testPathPattern="workspace-budget|businesses"</automated>
  </verify>
  <done>
...
<verification>
```bash
# Budget and workspace tests pass
cd /Users/arana/dev/ankit/mission-control && npm test -- --testPathPattern="workspace-budget|businesses"
