...
**NOTE ON ROLE SYSTEM:** This plan introduces the NEW 4-role hierarchy (`admin > agent > collaborator > viewer`). The existing schema uses OLD roles (`owner/admin/member`). Plan 02-02 still bridges to the OLD roles via `requireAdmin()` from `organizationMembers.ts`. The `adminMutation` builder in `functions.ts` (Task 2) handles the transition by mapping legacy `owner` role to admin level. A dedicated role migration (updating existing `organizationMembers` records from old→new role values) is deferred to a future phase. Until then, both role sets coexist: new code uses `workspaceAuth.ts` hierarchy, legacy guards accept `owner`/`admin`/`member`.
  </action>
  <verify>
