...
**IMPORTANT:** Do NOT break the existing workspace switching flow. The `setCurrentWorkspace` function and localStorage persistence should work exactly the same.
  ...
  <verify>
    <automated>cd /Users/arana/dev/ankit/mission-control && npm run build && npm test -- --testPathPattern="WorkspaceProvider"</automated>
  </verify>
  <postcondition>
**Unit test for WorkspaceProvider membership filtering:**

Create or update `frontend/src/components/__tests__/WorkspaceProvider.test.tsx`:

1. **Renders workspaces from getMyWorkspaces (not getAll):** Mock `useQuery` to return 2 workspaces → verify both rendered in context
2. **Filters by membership:** Mock `useQuery(api.workspaces.getMyWorkspaces)` → verify `api.workspaces.getAll` is NOT called
3. **hasNoAccess is true when no memberships:** Mock empty workspace list → verify `hasNoAccess` is true in context
4. **Includes role field from membership:** Mock workspace with `role: "admin"` → verify role accessible in context

Use React Testing Library + mock Convex `useQuery`.
  </postcondition>
  <done>
    ...
    - WorkspaceProvider unit tests pass (membership filtering verified)
    - Build passes
  </done>
</task>
