/**
 * GET/POST /api/v1/workspaces/{workspaceId}/epics
 *
 * Epic management endpoints — v1 REST API standardized format
 *
 * GET - List epics with pagination
...
const log = createLogger("api:v1:workspaces:epics");
const convex = new ConvexHttpClient(process.env.NEXT_PUBLIC_CONVEX_URL!);

/**
 * GET /api/v1/workspaces/{workspaceId}/epics
 ...
 *
 * Response: RFC 9457 compliant paginated epic list
 */
export async function GET(
  ...
  try {
    ...
    log.error("Error listing workspace epics", { error, requestId });
...
/**
 * POST /api/v1/workspaces/{workspaceId}/epics
 * Create new epic
 *
 * Body:
 * {
 ...
 * }
 *
 * Response: RFC 9457 compliant with created epic
 */
export async function POST(
  ...
  try {
    ...
    if (!body.title || typeof body.title !== "string") {
      throw new ValidationError("Epic title is required", pathname);
    }
    ...
    // Call Convex — create epic
    const epic = await convex.mutation(api.epics.createEpic, {
      workspaceId,
      title: body.title,
    ...
    log.error("Error creating epic", { error, requestId });
