/**
 * WorkspaceFilter Component Tests
 *
 * Tests for workspace filter dropdown used on global tabs
 * Validates: filter UI, workspace selection, filter state management
 */
...
class WorkspaceFilterMock {
  ...
  constructor(
    ...
    this.selectedWorkspaceId = currentFilter;
  ...
  select(workspaceId: string | null): void {
    ...
    this.callHistory.push(workspaceId);
    this.onFilterChangeMock(workspaceId);
    this.closeDropdown();
  }
  ...
  getDisplayText(): string {
    ...
    const workspace = this.businesses.find((b) => b._id === this.selectedWorkspaceId);
    return workspace ? `${ workspace.emoji} ${ workspace.name}` : "All es";
  }
...
describe("WorkspaceFilter Component", () => {
  let mockes: [];
  let mockCallback: (id: string | null) => void;
  let component: WorkspaceFilterMock;
  ...
  describe("Rendering", () => {
    it("should render filter dropdown", async () => {
      // Arrange: WorkspaceFilter rendered with businesses
      component = new WorkspaceFilterMock(
        mockes,
        null,
        mockCallback
      );
      ...
      expect(component.getDisplayText()).toBe("All es");
    });

    it("should display 'All es' option by default", async () => {
      // Expected: default display text is "All es"
      component = new WorkspaceFilterMock(
        mockes,
        null,
        mockCallback
    ...
    it("should list all businesses in dropdown", async () => {
      ...
      expect(options[0].name).toBe("All es");
    });

    it("should show workspace emoji and name in list", async () => {
      // Expected: each workspace shows emoji + name (e.g., "🚀 Mission Control HQ")
      component = new WorkspaceFilterMock(
        mockes,
        null,
        mockCallback
    ...
    it("should use workspace color for visual distinction", async () => {
      // Expected: workspace name has color styling from  workspace.color
      component = new WorkspaceFilterMock(
        mockes,
        "biz_1",
        mockCallback
  ...
  describe("Filter Selection", () => {
    it("should call onFilterChange(null) when 'All es' selected", async () => {
      // Arrange
      component = new WorkspaceFilterMock(
        mockes,
        "biz_1",
      ...
      // Act: click dropdown, select "All es"
    ...
    it("should call onFilterChange(workspaceId) when specific workspace selected", async () => {
      // Arrange
      component = new WorkspaceFilterMock(
        mockes,
        null,
  ...
  describe("Filter State Management", () => {
    ...
    it("should show 'All es' when currentFilter is null", async () => {
      // Arrange: currentFilter = null
      component = new WorkspaceFilterMock(mockes, null, mockCallback);

      // Expected: "All es" display
      expect(component.getDisplayText()).toBe("All es");
    });

    it("should show correct workspace when currentFilter is workspaceId", async () => {
    ...
    it("should handle invalid currentFilter gracefully", async () => {
      ...
      // Expected: defaults to "All es" or shows error
      const text = component.getDisplayText();
      expect(
        text === "All es" || text.includes("nonexistent_id")
      ).toBe(true);
    });
    ...
    it("should update when currentFilter prop changes", async () => {
      ...
      component = new WorkspaceFilterMock(mockes, "biz_1", mockCallback);
  ...
  describe("UI Interactions", () => {
    ...
    it("should support keyboard selection (Enter key)", async () => {
      ...
      // Expected: workspace selected, onFilterChange called
      expect(component.wasCalledWith("biz_1")).toBe(true);
    });
  ...
  describe("Integration with Global Tabs", () => {
    ...
    it("should NOT appear in Calendar tab", async () => {
      ...
      // In integration test: would verify Calendar route doesn't render WorkspaceFilter component
    });
    ...
    it("should clear filter when 'All es' selected", async () => {
      // Arrange
      component = new WorkspaceFilterMock(
        mockes,
        "biz_2",
